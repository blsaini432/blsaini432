...
public partial class Root_UC_Creditcard : System.Web.UI.UserControl
{
                ...
                if (ViewState["MemberMsrNo"] != null)
                {
                    ...
                    string MemberTypeID = Convert.ToString(dts.Rows[0]["MemberTypeID"]);
                        ...
                        // if (result > 0)
                        {
                            ...
                            string ITR = uploadPanImage(FUP_ITR);
                          ...
                          //  _list.Add(new ParmList() { name = "@EmailId ", value = txtEmail.Text });
                            ...
                            _list.Add(new ParmList() { name = "@bankstatement ", value = bankstatementfinacialyear });
                            _list.Add(new ParmList() { name = "@itr ", value = ITR });
                           // _list.Add(new ParmList() { name = "@form16 ", value = form16 });
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                if (Convert.ToInt32(dt.Rows[0]["credit_id"]) > 0)
                                {
                                    ...
                                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Your Request Save Successfully.!');location.replace('CreditCardReport.aspx');", true);
                                }
                            }
                            else
                            {
