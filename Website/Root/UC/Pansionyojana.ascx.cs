...
public partial class Root_UC_Pansionyojana : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (ViewState["MemberMsrNo"] != null)
            {
                ...
                decimal Amount = 0;
                Amount = Convert.ToDecimal(cls.select_data_scalar_double("select FeeAmount from [tbl_GSTFeeSettings] where memberTypeID=" + Convert.ToInt32(MemberTypeID) + " and actiontype='Pansion Yojna'"));
                    ...
                    if (memberID != "" && Amount > 0)
                    {
                        ...
                        if (result > 0)
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                ...
                                {
                                    clsm.Wallet_MakeTransaction(memberID, Convert.ToDecimal("-" + Amount), "Dr", "Pansionyojana Request TxnID:-" + TxnID);
                                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Your Request Save Successfully.!');location.replace('Pansionyojanareport.aspx');", true);
                                }
                            }
                            else
                            {
