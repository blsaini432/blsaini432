...
public partial class Root_UC_FirmmsmeRegistration : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (ViewState["MemberMsrNo"] != null)
            {
                ...
                decimal Amount = 0;
                Amount = Convert.ToDecimal(cls.select_data_scalar_double("select FeeAmount from [tbl_GSTFeeSettings] where memberTypeID=" + Convert.ToInt32(MemberTypeID) + " and actiontype='Firm Registration Fee'"));
                ...
                if (ViewState["MemberMsrNo"] != null)
                {
                    ...
                    if (memberID != "" && Amount > 0)
                    {
                        ...
                        if (result > 0)
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                if (Convert.ToInt32(dt.Rows[0]["firm_id"]) > 0)
                                {
                                    // clsm.Wallet_MakeTransaction(Session["adminmemberid"].ToString(), Convert.ToDecimal("-" + adminamount), "Dr", "ITR Request TxnID:-" + TxnID);
                                    clsm.Wallet_MakeTransaction(memberID, Convert.ToDecimal("-" + Amount), "Dr", "Firm/msme Request TxnID:-" + TxnID);
