...
using System.Data;
using System.IO;
public partial class Root_UC_PanCorrectionForm : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (ViewState["MemberMsrNo"] != null)
            {
                ...
                decimal Amount = 0;
                Amount = Convert.ToDecimal(cls.select_data_scalar_double("select  isnull(Amount,0.00) as Amount from[nsdl_couponfeesettings] where PackageID=" + Convert.ToInt32(dts.Rows[0]["PackageID"])));
                            ...
                            if (Form != "")
                            {
                                ...
                                if (txtTxnNo.Text == "" && txtTxnNo.Text == string.Empty)
                                {
                                  ...
                                  //  _list.Add(new ParmList() { name = "@DOBImage", value = Dobimg });
                                    _list.Add(new ParmList() { name = "@Correctionform", value = Form });
                                    ...
                                    _list.Add(new ParmList() { name = "@SurrenderPan1", value = txt_surendrapan1.Text });
                                    _list.Add(new ParmList() { name = "@SurrenderPan2", value = txt_surendrapan2.Text });
                                    _list.Add(new ParmList() { name = "@SurrenderPan3", value = txt_surendrapan3.Text });
                                    ...
                                    if (dt.Rows.Count > 0)
                                    {
                                        ...
                                        {
                                            clsm.Wallet_MakeTransaction(memberID, Convert.ToDecimal("-" + Amount), "Dr", "Correction PanCard Request TxnID:-" + TxnID);
                                            ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Your Request Save Successfully.!');location.replace('PanCorrection_Report.aspx');", true);
                                        }
                                    }
                                    else
                                    {
                                ...
                                else
                                {
                                 ...
                                 //   _list.Add(new ParmList() { name = "@DOBImage", value = Dobimg });
                                   ...
                                   // _list.Add(new ParmList() { name = "@formA", value = FormA });
                                    ...
                                    if (dt.Rows.Count > 0)
                                    {
                                        ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Data Updated Successfully.!');location.replace('PanCorrectionForm.aspx.aspx');", true);
                                    }
                                    else
                                    {
                                        ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Some Problem In Request processing.Please try Again Or Contact To Admin.!');", true);
