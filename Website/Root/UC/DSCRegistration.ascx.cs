...
public partial class Root_UC_DSCRegistration : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (ViewState["MemberMsrNo"] != null)
            {
                ...
                decimal Amount = 0;
                Amount = Convert.ToDecimal(cls.select_data_scalar_double("select FeeAmount from [tbl_GSTFeeSettings] where memberTypeID=" + Convert.ToInt32(MemberTypeID) + " and actiontype='DSC Registration Fee'"));
                        ...
                        if (result > 0)
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                if (Convert.ToInt32(dt.Rows[0]["food_id"]) > 0)
                                {
                                    ...
                                    clsm.Wallet_MakeTransaction(memberID, Convert.ToDecimal("-" + Amount), "Dr", "DSC Request TxnID:-" + TxnID);
