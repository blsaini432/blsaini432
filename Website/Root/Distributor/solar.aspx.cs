...
public partial class Root_Distributor_Solar : System.Web.UI.Page
{
    #region Access_Class
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    DataTable dtMember = (DataTable)Session["dtDistributor"];
                    {
                        ...
                        decimal Amount = 0;
                        Amount = Convert.ToDecimal(cls.select_data_scalar_double("select  isnull(Amount,0.00) as Amount from[manageservices] where PackageID='" + Convert.ToInt32(PackageID) + "'and servicename='Solar energy'"));
