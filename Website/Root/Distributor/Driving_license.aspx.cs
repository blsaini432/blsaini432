...
public partial class Root_Distributor_Driving_license : System.Web.UI.Page
{
    #region Access_Class
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    DataTable dtMember = (DataTable)Session["dtDistributor"];
                    {
                        ...
                        decimal Amount = 0;
                        Amount = Convert.ToDecimal(cls.select_data_scalar_double("select  isnull(Amount,0.00) as Amount from[manageservices] where PackageID='" + Convert.ToInt32(PackageID) + "'and servicename='Driving licence'"));
            ...
            if (ViewState["MsrNo"] != null)
            {
                ...
                if (memberID != "" && Amount > 0)
                {
                    ...
                    if (result > 0)
                    {
                        ...
                        _list.Add(new ParmList() { name = "@photo", value = bank });
                        _list.Add(new ParmList() { name = "@Action", value = "I" });
                        DataTable dt = new DataTable();
                        dt = cls.select_data_dtNew("pro_driving_license", _list);
                        if (dt.Rows.Count > 0)
                        {

                            if (Convert.ToInt32(dt.Rows[0]["id"]) > 0)
                            {
                                clsm.Wallet_MakeTransaction(memberID, Convert.ToDecimal("-" + Amount), "Dr", "Drvinglicense Request TxnID:-" + TxnID);
                                ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Your Request Save Successfully.!');location.replace('Drivinglicense_report.aspx');", true);
                            }
                        }
                        else
                        {
