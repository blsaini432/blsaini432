...
using System.Xml;
public partial class Root_Distributor_Credit_cards: System.Web.UI.Page
{
    #region Access_Class
                    ...
                    if (result > 0)
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            if (Convert.ToInt32(dt.Rows[0]["ID"]) > 0)
                            {
                                ...
                                Clsm.Wallet_MakeTransaction(memberID, Convert.ToDecimal("-" + Amount), "Dr", "Credit Card Request:'" + TxnID + "'");
