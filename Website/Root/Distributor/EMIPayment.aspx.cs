...
using System.Xml;
public partial class Root_Distributor_EMIPayment : System.Web.UI.Page
{
            ...
            if (ViewState["MsrNo"] != null)
            {
                ...
                if (memberID != "" && Amount > 0)
                {
                    ...
                    if (result > 0)
                    {
                        ...
                        DataTable dt = new DataTable();
                        dt = Cls.select_data_dtNew("Proc_EMIPAYMENT", _list);
                        if (dt.Rows.Count > 0)
                        {
...

                            {
                              ...
                              //  valueArray[2] = txt_cardname.Text;
                               // SendWithVarpan(ViewState["mobile"].ToString(), 1, valueArray);
                                Clsm.Wallet_MakeTransaction(memberID, Convert.ToDecimal("-" + Amount), "Dr", "Life insurance Payment Request:'" + TxnID+"'");
                               // Cls.select_data_dt(@"EXEC ProcMLM__EWalletTransaction '" + ViewState["MemberId"].ToString() + "','" + NetAmount + "','Cr','Life Insurance Commission:" + TxnID + "'");
                                ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Transaction Successfull!');location.replace('EMIPayment_report.aspx');", true);
                            }
                        }
                        else
                        {
    ...
    public double TotupAmount(double amount, string memberid)
    {
        ...
        if (amount > 0)
        {
            ...
            dtsr = cls.select_data_dt(@"EXEC Set_EzulixDmr @action='EMI',@amount='" + Convert.ToDecimal(amount) + "',@packageid=" + PackageID + "");
    ...
    {
        "Zero",
        "Dear Customer, your EMI payment amount Rs.@v0@ and Policy Number @v1@. and policy Holder Name @v2@"//1
    };
