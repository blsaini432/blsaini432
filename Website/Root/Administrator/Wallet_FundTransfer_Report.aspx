<%@ Page Title="" Language="C#" MasterPageFile="~/Root/Administrator/AdminMaster.master"
    AutoEventWireup="true" CodeFile="Wallet_FundTransfer_Report.aspx.cs" Inherits="Root_Admin_Wallet_FundTransfer_Report" %>
    ...
    <style>
      ...
      <script>
          ...
          app.controller("myCntrl", function ($scope, $http, $timeout, $filter) {
              ...
              $scope.fillWalletTransferReport = function () {
                  $("#loader").show();
                  var httpreq = {
                      method: 'POST',
                      url: '../Administrator/Wallet_FundTransfer_Report.aspx/fillWalletTransferReport',
                      headers: {
                  ...
                  $http(httpreq).success(function (response) {
                      $("#loader").hide();
                      $scope.wallettransferreport = response.d;
                  })
              };
              $scope.fillWalletTransferReport();
              $("#loader").hide();
              //

              $scope.fillWalletTransferReportbydate = function (fdate, tdate) {
                  $("#loader").show();
                  var httpreq = {
                      method: 'POST',
                      url: '../Administrator/Wallet_FundTransfer_Report.aspx/fillWalletTransferReportbydate',
                      headers: {
                          'Content-Type': 'application/json; charset=utf-8',
                          'dataType': 'json'
                      },
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <div class="content-wrapper">
        <div class="page-header">
            <h3 class="page-title">Wallet Transfer Report
            </h3>
        </div>
                        ...
                        <div class="container" ng-app="myApp" ng-controller="myCntrl">
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group row">
                                        <div class="col-sm-6">
                                            <a href="#" ng-click="fillWalletTransferReportbydate(from_date,to_date)" class="btn btn-primary">Search &gt;&gt;</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="table-responsive">
                                    <table id="tblEwalletsummary" class="table table-bordered table-hover">
                                        <thead>
                                            ...
                                            <tr dir-paginate="x in wallettransferreport|filter:search|itemsPerPage:pageSize">
                                                <td>{{$index + 1}}</td>
                                                <td>{{x.memberID}}</td>
                                                <td>{{x.MemberName}}</td>
                                                <td>{{x.mobile}}</td>
                                             ...
                                             <tr>
                                                <td ng-show="wallettransferreport.length==0" colspan="14">
