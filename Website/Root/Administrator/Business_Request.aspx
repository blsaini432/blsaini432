<%@ Page Title="" Language="C#" MasterPageFile="~/Root/Administrator/AdminMaster.master" AutoEventWireup="true" CodeFile="Business_Request.aspx.cs" Inherits="Root_Admin_Business_Request" %>
    ...
    <script>
        ...
        app.controller("myCntrl", function ($scope, $http, $timeout, $filter) {
            ...
            $scope.filldmrnewreport = function () {
                ...
                var httpreq = {
                    method: 'POST',
                    url: '../Administrator/Business_Request.aspx/Business_Request',
                    headers: {
            ...
            $scope.filldmrnewreportbydate = function (fdate, tdate) {
                ...
                var httpreq = {
                    method: 'POST',
                    url: '../Administrator/Business_Request.aspx/Business_Requestbydate',
                    headers: {
                        'Content-Type': 'application/json; charset=utf-8',
                        'dataType': 'json'
                    },
            ...
            function ApproveBRequest(id) {
                ...
                $http({
                    url: '../Administrator/Business_Request.aspx/ApproveRequest',
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json; charset=utf-8',
                        'dataType': 'json'
            ...
            function RejectBRequest(id, mobile, date, amount) {
                ...
                var amount = amount;
                $http({
                    url: '../Administrator/Business_Request.aspx/RejectRequest',
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/json; charset=utf-8',
                        'dataType': 'json'
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <div class="content-wrapper">
        <div class="page-header">
            <h3 class="page-title">Business With US Request
            </h3>
        </div>
                                    ...
                                    <table id="tblEwalletsummary" class="table table-bordered table-hover">
                                        <thead>
                                            ...
                                            <tr dir-paginate="x in dmrnewreport|filter:search|itemsPerPage:pageSize">
                                                ...
                                                <td>{{x.MemberID}}</td>
                                                <td>{{x.MemberName}}</td>
                                                <td>{{x.Business_Partner}}</td>
                                                <td>{{x.mobile}}</td>
                                                <td>{{x.Present_Business}}</td>
                                                <td>{{x.Experinces}}</td>
                                                <td>{{x.Present_income}}</td>
                                                <td>{{x.invert_rent}}</td>
                                                <td>{{x.Business}}</td>
