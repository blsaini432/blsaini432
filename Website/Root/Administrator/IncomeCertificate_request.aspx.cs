...
public partial class Root_Admin_IncomeCertificate_request : System.Web.UI.Page
{
    ...
    protected void btn_export_Click(object sender, EventArgs e)
    {
        ...
        if (fromdate != "" && todate != "")
        {
            ...
            dtExport = cls.select_data_dtNew("Set_IncomeCertificate_report", _lstparm);
    ...
    public static List<Customer> pancardreport()
    {
        ...
        _lstparm.Add(new ParmList() { name = "@action", value = "admin" });
        dtEWalletTransaction = cls.select_data_dtNew("Set_IncomeCertificate_report", _lstparm);
    ...
    public static string ApproveRequest(string fundid)
    {
        ...
        string actions = "";
            ...
            dt = cls.select_data_dt("select * from Tbl_IncomeCertificate where txnid='" + id + "' and statu='Pending'");
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                cls.select_data_dt(@"Update Tbl_IncomeCertificate set statu='SUCCESS' Where  txnid='" + id + "'");
    ...
    public static string RejectRequest(string fundid)
    {
        ...
        string txn = fundid;
        DataTable dt = new DataTable();
        dt = cls.select_data_dt("select * from Tbl_IncomeCertificate where txnid='" + txn + "' and statu='Pending'");
        string status = string.Empty;
        string txnid = string.Empty;
        string totalamt = string.Empty;
        string memberid = string.Empty;
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            cls_myMember clsm = new cls_myMember();
           clsm.Wallet_MakeTransaction(memberid, Convert.ToDecimal(Convert.ToDecimal(totalamt)), "Cr", "Reverse Income Certificate TxnID:-" + txnid);
            cls.select_data_dt(@"Update Tbl_IncomeCertificate set statu='Fail' Where  txnid='" + txn + "'");
            actions = "success";
            return actions;
        }
        else
