...
public partial class Root_Admin_Drivinglicense_request : System.Web.UI.Page
{
    ...
    public static List<Customer> fillfundrequest()
    {
        ...
        dtEWalletTransaction = cls.select_data_dtNew("pro_driving_license", _lstparm);
    ...
    protected void btn_export_Click(object sender, EventArgs e)
    {
        ...
        if (fromdate != "" && todate != "")
        {
            ...
            _lstparm.Add(new ParmList() { name = "@datefrom", value = changedatetommddyy(fromdate) });
            _lstparm.Add(new ParmList() { name = "@dateto", value = changedatetommddyy(todate) });
            dtExport = cls.select_data_dtNew("pro_driving_license", _lstparm);
            if (dtExport.Rows.Count > 0)
            {
                Common.Export.ExportToExcel(dtExport, "drivinglicense");
            }
            else
            {
    ...
    protected void btnSubmit_Reject(object sender, EventArgs e)
    {
        ...
        dt = cls.select_data_dt("select * from Tbl_drivinglicense where txnid='" + txnid + "' and status='pending'");
        ...
        if (adminstatus != "" && txnid != "")
        {
            ...
            cls.select_data_dt(@"Update Tbl_drivinglicense set status='failed', Adminstatus='" + adminstatus + "' Where  txnid='" + txnid + "'");
            clsm.Wallet_MakeTransaction(memberid, Convert.ToDecimal(Convert.ToDecimal(totalamt)), "Cr", "Reverse drivinglicense  TxnID:-" + txnid);
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Update successfully!');location.replace('Drivinglicense_request.aspx');", true);
        }
        else
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('please upload  remark!');location.replace('Drivinglicense_request.aspx');", true);
        }
    ...
    protected void btnSubmit_Apprvo(object sender, EventArgs e)
    {
        ...
        if (adminstatus != "" && txnid != "")
        {
            cls.select_data_dt(@"Update Tbl_drivinglicense set status='success', Adminstatus='" + adminstatus + "' Where  txnid='" + txnid + "'");
