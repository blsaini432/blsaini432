...
public partial class Root_Admin_AepsPayoutSurcharge : System.Web.UI.Page
{
    #region [Properties]
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        #region [Insert]
        Int32 intresult = 0;
        int i = objconnection.delete_data("delete from m_aeps_payout_Surcharge where PackageID=" + Convert.ToInt32(ddlPackage.SelectedValue));
        DataTable dt = new DataTable();
        dt = (DataTable)Session["mySurcharge"];
        for (int j = 0; j < dt.Rows.Count; j++)
        {
            intresult = intresult + objconnection.insert_data("insert into m_aeps_payout_Surcharge (packageid, startval, endval, surcharge, isFlat) values (" + ddlPackage.SelectedValue + ",'" + dt.Rows[j][2].ToString() + "','" + dt.Rows[j][3].ToString() + "','" + dt.Rows[j][4].ToString() + "','" + dt.Rows[j][5].ToString() + "')");
        }
        if (intresult > 0)
        {
    ...
    protected void ddlPackage_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            dtOperator = objconnection.select_data_dt("Exec MM_m_aeps_payout_Surcharge 0,'" + ddlPackage.SelectedValue + "','0',0");
            DataView dv = dtOperator.DefaultView;
            DataTable sortedDT = dv.ToTable();
