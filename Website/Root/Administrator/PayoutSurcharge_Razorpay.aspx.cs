...
public partial class Root_Admin_PayoutSurcharge_Razorpay : System.Web.UI.Page
{
    #region [Properties]
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        #region [Insert]
        Int32 intresult = 0;
        int i = objconnection.delete_data("delete from Razorpay_payout_Surcharge where PackageID=" + Convert.ToInt32(ddlPackage.SelectedValue));
        DataTable dt = new DataTable();
        dt = (DataTable)Session["mySurcharge"];
        for (int j = 0; j < dt.Rows.Count; j++)
        {
            intresult = intresult + objconnection.insert_data("insert into Razorpay_payout_Surcharge (packageid, startval, endval, surcharge, isFlat,modetype) values (" + ddlPackage.SelectedValue + ",'" + dt.Rows[j][2].ToString() + "','" + dt.Rows[j][3].ToString() + "','" + dt.Rows[j][4].ToString() + "','" + dt.Rows[j][5].ToString() + "','" + dt.Rows[j][6].ToString() + "')");
        }
        if (intresult > 0)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Success|Plan save successfully');location.replace('PayoutSurcharge_mysun.aspx');", true);        
        }
        else
        {
    ...
    protected void ddlPackage_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            dtOperator = objconnection.select_data_dt("Exec Razorpay_payout_Surcharg 0,'" + ddlPackage.SelectedValue + "','0',0");
            DataView dv = dtOperator.DefaultView;
            DataTable sortedDT = dv.ToTable();
    ...
    protected void btnadd_Click(object sender, EventArgs e)
    {
        if (Session["mySurcharge"] != null)
        {
            ...
            if (hdnidd.Value == "")
            {
              ...
              //  NewRow[6] = Transactionlist.SelectedValue;
            ...
            else
            {
              ...
              //  dt.Rows[Convert.ToInt32(hdnidd.Value)].SetField(5, Transactionlist.SelectedValue);
                dt.Rows[Convert.ToInt32(hdnidd.Value)].SetField(6, chkflatd.Checked);
                Session["mySurcharge"] = dt;
                gvOperator.DataSource = dt;
                gvOperator.DataBind();
